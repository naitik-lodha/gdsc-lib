...
router.get("isbn/:isbn", (req, res) => {
  ...
  if (!book)
    res
      .status(404)
      .send("The book of ISBN " + req.params.isbn + " is not available");
  res.send(book);
});
...
router.get("title/:title", (req, res) => {
  ...
  if (!book)
    res
      .status(404)
      .send("Sorry we do not have any book by the name  " + req.params.title);
  res.send(book);
});
...
router.get("/author/:author", (req, res) => {
  ...
  if (!book)
    res
      .status(404)
      .send("Sorry we do not have any book written by " + req.params.author);
  res.send(book);
});
